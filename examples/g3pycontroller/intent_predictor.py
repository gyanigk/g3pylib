...
class IntentPredictor:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-4o"):
        # Try to load API key from key.json if not provided
        if api_key is None:
        ...
        self.prediction_interval = 1.0  # Reduced from 2.0 seconds
        self.max_tokens = 500  # Further reduced for faster responses
        self.max_completion_tokens = 500  # OpenAI API parameter for max output tokens
        self.temperature = 0.3  # Default temperature
        self.use_responses_api = False  # Disable hypothetical Responses API for GPT-5 (not available)
        self.max_image_size = 384  # Reduced from 512 for faster processing
        self.jpeg_quality = 75  # Reduced quality for smaller payload
        ...
        # ]
        # self.default_candidates = [
        #     "pink cup", "white cup", "orange juice", "steel cup",
        #     "yellow packet", "half half milk", "silk milk", "whole milk",
        #     "soy milk", "diet coke soda can", "coke soda can",  "sugar packet", "observing"
        # ]
        
        self.default_candidates = [
            "white fan", "notepad", "whole milk", "half and half milk","small cup",
            "diet coke", "screwdriver", "marker pen", "big cup",
            "black tape", "wooden block", "coke zero", "observing"
        ]
        ...
        self._cached_scene_b64 = None
        self._cached_heatmap_b64 = None
        self._cache_threshold = 0.95  # Similarity threshold for cache reuse
        
        # Thread pool for concurrent processing
        self._executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="intent_pred")
        
        # Latest prediction storage for UI updates
        self._latest_prediction = None
        self._prediction_lock = threading.Lock()
        self._last_ui_update_timestamp = 0  # Track when UI was last updated
        ...
        self._all_predictions_metadata = []  # Initialize metadata storage
        
        # Ultra-optimized system prompt for fastest responses
        # self.system_prompt = """Analyze scene + gaze heatmap. Pick ONE action from candidates list. Return JSON: {"prediction": "action", "reasoning": "brief"}"""
        self.system_prompt = """
    ...
    def _load_api_key_from_file(self) -> Optional[str]:
        """Load OpenAI API key from key.json file."""
        try:
            # Try to find key.json in the project root (go up from current file location)
            current_file = Path(__file__)
            project_root = current_file.parent.parent.parent.parent.parent  # Go up to project root
            key_file = project_root / "key.json"
            
            if key_file.exists():
                with open(key_file, 'r', encoding='utf-8') as f:
                    key_data = json.load(f)
                    api_key = key_data.get("openai_key")
                    if api_key:
                        logging.info(f"Loaded OpenAI API key from {key_file}")
                        return api_key
                    else:
                        logging.warning(f"No 'openai_key' field found in {key_file}")
            else:
                logging.info(f"Key file not found at {key_file}")
        except Exception as e:
            logging.error(f"Failed to load API key from file: {e}")
    ...
    def set_image_optimization(self, max_size: int = 512, quality: int = 85):
        """Set image optimization parameters for faster processing."""
        self.max_image_size = max(128, min(max_size, 1024))  # Limit between 128 and 1024
        self.jpeg_quality = max(50, min(quality, 95))  # Quality between 50-95
        logging.info(f"Image optimization set to max_size={self.max_image_size}, quality={self.jpeg_quality}")
    ...
    def _prepare_frame_for_saving(self, frame: np.ndarray) -> np.ndarray:
        """Prepare frame for saving (convert BGR to RGB - frame is already correctly oriented)."""
        try:
            if len(frame.shape) == 3 and frame.shape[2] == 3:
                # Convert BGR to RGB for proper color saving
                if frame.dtype == np.uint8:
                    # Manual BGR to RGB conversion - frame is already properly oriented from main app
    ...
    def _resize_frame_if_needed(self, frame: np.ndarray, max_size: int = None) -> np.ndarray:
    ...
    def _calculate_frame_hash(self, frame: np.ndarray) -> str:
        """Calculate a hash of the frame for similarity detection."""
        try:
            # Use a small downsampled version for fast hashing
            small_frame = self._resize_frame_if_needed(frame, max_size=64)
            return hashlib.md5(small_frame.tobytes()).hexdigest()
        except Exception as e:
            logging.error(f"Failed to calculate frame hash: {e}")
            return ""

    def _frames_similar(self, hash1: str, hash2: str) -> bool:
        """Check if two frame hashes are similar enough to reuse cache."""
        if not hash1 or not hash2:
            return False
        # For now, use exact match - could implement more sophisticated similarity
        return hash1 == hash2

    def _frame_to_base64(self, frame: np.ndarray, cache_key: str = None) -> str:
        """Convert numpy frame to base64 encoded JPEG with caching for faster processing."""
        try:
            ...
            # Convert to base64 using JPEG for smaller size and faster upload
            buffer = io.BytesIO()
            ...
            if cache_key:
                ...
                if len(self._frame_cache) > 10:
                    ...
                    keys_to_remove = list(self._frame_cache.keys())[:5]
    ...
    def _make_chat_completions_request(self, scene_b64: str, heatmap_b64: str, user_text: str) -> Dict[str, Any]:
        """Make a Chat Completions API request (fallback for image support)."""
        try:
            ...
            request_params = {
                ...
                "messages": [
                    {"role": "system", "content": self.system_prompt},
                    {
                        ...
                        "content": [
                            {"type": "text", "text": user_text},
                            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{scene_b64}", "detail": "low"}},
                            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{heatmap_b64}", "detail": "low"}},
                        ],
                    },
                ],
            }
            ...
            # Note: The 'reasoning' and 'text' parameters are not supported in Chat Completions API
            # Only add temperature for non-GPT-5 models
            if "gpt-5" not in self.model.lower():
                # For other models, add temperature if it's not the default
            ...
            if not response_content or response_content.strip() == "":
                ...
                return {"prediction": "observing", "reasoning": "Empty API response"}
            
            # Fast response cleaning - optimized for JSON extraction
            cleaned_response = response_content.strip()
            start_idx = cleaned_response.find("{")
            end_idx = cleaned_response.rfind("}") + 1
            
            if start_idx == -1 or end_idx <= start_idx:
                # No JSON found, try to extract any meaningful text
                logging.warning(f"No JSON found in response: {cleaned_response}")
                # Try to find a prediction in the text
                for candidate in self.current_candidates:
                    if candidate.lower() in cleaned_response.lower():
                        return {"prediction": candidate, "reasoning": "Extracted from text response"}
                return {"prediction": "observing", "reasoning": "No valid JSON or recognizable prediction"}
            ...
            if not result or not isinstance(result, dict):
                logging.warning(f"Empty or invalid JSON object: {result}")
                return {"prediction": "observing", "reasoning": "Invalid JSON response format"}
            
            if "prediction" not in result:
                logging.warning(f"No prediction field in response: {result}")
                # Try to find any candidate action in the response values
                for key, value in result.items():
                    if isinstance(value, str):
                        for candidate in self.current_candidates:
                            if candidate.lower() in value.lower():
                                return {"prediction": candidate, "reasoning": f"Extracted from {key}: {value}"}
                return {"prediction": "observing", "reasoning": f"No prediction field found: {result}"}
            
            # Add default reasoning if missing
            if "reasoning" not in result:
                result["reasoning"] = "fast prediction"
            
            # Validate prediction is in candidates
            if result["prediction"] not in self.current_candidates:
                logging.warning(f"Prediction '{result['prediction']}' not in candidates, defaulting to observing")
                result["prediction"] = "observing"
                result["reasoning"] = f"Invalid prediction, defaulted to observing"
            
            return result
            
        except json.JSONDecodeError as e:
            logging.error(f"JSON parse error: {e}, response: {cleaned_response}")
            return {"prediction": "observing", "reasoning": f"Parse error: {str(e)[:50]}"}
        except Exception as e:
            logging.error(f"Prediction request failed: {e}")
            return {"prediction": "observing", "reasoning": f"API error: {str(e)[:50]}"}
    
    def predict_intent_fast(self) -> Optional[Dict[str, Any]]:
        """Ultra-fast prediction using caching and optimization."""
        ...
        try:
            ...
            # Calculate frame hashes for similarity detection
            scene_hash = self._calculate_frame_hash(scene_frame)
            heatmap_hash = self._calculate_frame_hash(heatmap_frame)
            combined_hash = f"{scene_hash}_{heatmap_hash}"
            
            # Check if frames are similar to last prediction - if so, reuse result
            if (self._last_frame_hash and 
                self._frames_similar(combined_hash, self._last_frame_hash) and 
                self.prediction_history):
                
                # Reuse last prediction with updated timestamp
                last_prediction = self.prediction_history[-1].copy()
                last_prediction["timestamp"] = current_time
                last_prediction["duration_ms"] = 0  # Cached result
                last_prediction["cached"] = True
                
                with self._prediction_lock:
                    self._latest_prediction = last_prediction
                
                logging.debug(f"Fast cached prediction: {last_prediction['prediction']} (cached)")
                return last_prediction
            
            # Convert frames to base64 with caching
            scene_b64 = self._frame_to_base64(scene_frame, f"scene_{scene_hash}")
            heatmap_b64 = self._frame_to_base64(heatmap_frame, f"heatmap_{heatmap_hash}")
            ...
            # Update frame hash for next comparison
            self._last_frame_hash = combined_hash
            
            # Store latest prediction for UI updates
            with self._prediction_lock:
                self._latest_prediction = prediction
            
            logging.info(f"Fast intent prediction: {prediction['prediction']} - {prediction['reasoning']} (took {prediction_duration*1000:.2f}ms)")
            return prediction
            
        except Exception as e:
            logging.error(f"Failed to predict intent (fast): {e}")
            error_prediction = {
    ...
    async def predict_intent_ultra_fast(self) -> Optional[Dict[str, Any]]:
        """Ultra-fast async prediction with all optimizations enabled."""
        current_time = time.time()
        ...
        # Check timing - but allow more frequent predictions
        if current_time - self.last_prediction_time < 0.5:  # Reduced from prediction_interval
            return None
        ...
        try:
            # Run optimized prediction in executor
            loop = asyncio.get_event_loop()
            prediction = await loop.run_in_executor(
                self._executor, self._predict_intent_ultra_fast_sync, scene_frame, heatmap_frame, current_time
            )
            ...
            # Update last prediction time after successful prediction
            self.last_prediction_time = current_time
            
            logging.debug(f"Ultra-fast prediction: {prediction['prediction']} (took {prediction.get('duration_ms', 0):.1f}ms)")
            return prediction
            
        except Exception as e:
            logging.error(f"Failed to predict intent (ultra-fast): {e}")
            error_prediction = {"prediction": "observing", "reasoning": f"Prediction failed: {str(e)[:50]}", "timestamp": current_time}
            with self._prediction_lock:
                self._latest_prediction = error_prediction
            return error_prediction

    def _predict_intent_ultra_fast_sync(self, scene_frame: np.ndarray, heatmap_frame: np.ndarray, timestamp: float) -> Dict[str, Any]:
        """Ultra-fast synchronous prediction with all optimizations."""
        try:
            start_time = time.time()
            ...
            if (self._last_frame_hash and 
                ...
                last_prediction["duration_ms"] = 0.1  # Minimal cached result time
                last_prediction["cached"] = True
                
                logging.debug(f"Ultra-fast cached prediction: {last_prediction['prediction']}")
                return last_prediction
            
            # Save frames if enabled (non-blocking)
            if self.save_images_enabled:
                try:
                    self._save_prediction_frames(scene_frame, heatmap_frame, timestamp)
                except Exception as e:
                    logging.debug(f"Failed to save prediction frames: {e}")  # Non-critical
            
            # Convert frames to base64 with aggressive caching
            scene_b64 = self._frame_to_base64(scene_frame, f"scene_{scene_hash}")
            heatmap_b64 = self._frame_to_base64(heatmap_frame, f"heatmap_{heatmap_hash}")
            ...
            # Save prediction result to metadata if image saving is enabled (non-blocking)
            if self.save_images_enabled:
                try:
                    self._save_prediction_result(prediction)
                except Exception as e:
                    logging.debug(f"Failed to save prediction result: {e}")  # Non-critical
            
            return prediction
            
        except Exception as e:
            logging.error(f"Failed to predict intent in ultra-fast sync: {e}")
            return {
                "prediction": "observing", 
                "reasoning": f"Prediction failed: {str(e)[:50]}", 
                "timestamp": timestamp,
    ...
    async def predict_intent_async(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Wait for the specified interval after each prediction
    ...
    def get_latest_prediction_safe(self) -> Optional[Dict[str, Any]]:
        ...
        with self._prediction_lock:
            # Only return if this prediction hasn't been shown in UI yet
            if (self._latest_prediction and 
                self._latest_prediction.get('timestamp', 0) > self._last_ui_update_timestamp):
                
                # Mark this prediction as shown
                self._last_ui_update_timestamp = self._latest_prediction.get('timestamp', 0)
    ...
    def cleanup(self):
        """Clean up resources when predictor is no longer needed."""
        try:
            if hasattr(self, '_executor'):
                self._executor.shutdown(wait=True)
                logging.info("Intent predictor thread pool shut down")
        except Exception as e:
            logging.error(f"Error during intent predictor cleanup: {e}")
